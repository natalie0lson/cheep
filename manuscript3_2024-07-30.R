...
physeq@sam_data$loc.type.n2 <- gsub("/Market/Carcass", " ", physeq@sam_data$loc.type.n2)

pathseq@sam_data$loc.type.n2 <- gsub(", ", "\n", pathseq@sam_data$loc.type.n)
pathseq@sam_data$loc.type.n2 <- gsub("/Market/Carcass", " ", pathseq@sam_data$loc.type.n2)

ARGseq@sam_data$loc.type.n2 <- gsub(", ", "\n", ARGseq@sam_data$loc.type.n)
ARGseq@sam_data$loc.type.n2 <- gsub("/Market/Carcass", " ", ARGseq@sam_data$loc.type.n2)

clinseq@sam_data$loc.type.n2 <- gsub(", ", "\n", clinseq@sam_data$loc.type.n)
clinseq@sam_data$loc.type.n2 <- gsub("/Market/Carcass", " ", clinseq@sam_data$loc.type.n2)

#update names of custom colors 
names(custom_colors) <- gsub(", ", "\n", names(custom_colors))
names(custom_colors) <- gsub("Indigenous", "Local", names(custom_colors))
names(custom_colors) <- gsub("/Market/Carcass", " ", names(custom_colors))
...
table(phydat$phystack)
# Reorder the 'Category' variable based on the 'Value' variable
phydat$phystack <- factor(phydat$phystack, levels=c( 'Other', 'Chlamydiae',  'Bacteroidetes'  ,    'Firmicutes' , "Actinobacteria",  'Proteobacteria'  ))
...
table(phydat$phystack)
# Reorder the 'Category' variable based on the 'Value' variable
phydat$phystack <- factor(phydat$phystack, levels=c( "Other", "E. coli"   ,"E. hormaechei" ,    "K. pneumoniae","S. flexneri" ,  "S. enterica"    ))
...
arglist
table(car_args$clinical)
length(car_args$name2)
...
table(argdat$argstack)
# Reorder the 'Category' variable based on the 'Value' variable

argdat$argstack <- factor(argdat$argstack, levels=c( "Other","erm(B)" , "tet(39)" , "qnrS2", "sul2","aph(3'')-Ib"  ))
...
# Reorder the 'Category' variable based on the 'Value' variable
table(argdat$argstack)
argdat$argstack <- factor(argdat$argstack, levels=c( "Other", "erm(B)"  ,  "aph(3'')-Ib"  ,  "aph(6)-Id"   , "sul2" ,   "tet(L)"   ))
...
### DAA BACTERIA 
bac_car <- phycar %>% tax_glom("Phylum")

#DIFFERENTIAL ABUNDANCE BETWEEN BREEDS (AMONG PRODUCTION SITE)
daa_car <- differentialTest(formula = ~ breed,
                            phi.formula = ~ breed,
                            formula_null = ~ 1,
...
path_car <- pathcar %>% tax_glom("Genus")

#DAA ARGS
daa_path <- differentialTest(formula = ~ breed,
                            phi.formula = ~ breed,
                            formula_null = ~ 1,
                            phi.formula_null = ~ breed,
                            test = "Wald", boot = FALSE,
...
physeq_g <- tax_glom(phycar, "Phylum")

taxaorder <- unique(taxa_names(physeq_g))

phycar_com <- subset_samples(physeq_g, breed=="Commercial")
phycar_ind <- subset_samples(physeq_g, breed=="Local")

hm_phycar_com <-plot_heatmap(phycar_com, taxa.label= "Phylum", taxa.order=taxaorder, title = "Commercial", trans = log_trans(10), low="#e3fa9e", high="#59730d", na.value="white")  + theme_classic() + theme(text=element_text(size=50))+ scale_x_discrete(guide = guide_axis(angle = 40)) #+  scale_fill_gradient(limits=range(67,11661349),  low="#deebe4", high="#2c3831", na.value="white")
hm_phycar_com

hm_phycar_ind <-plot_heatmap(phycar_ind, taxa.label= "Phylum", taxa.order=taxaorder,  title = "Local",   trans = log_trans(10), low="#d6f5f4", high="#074e54", na.value="white") + theme_classic() + theme(text=element_text(size=50))+ scale_x_discrete(guide = guide_axis(angle = 40))#+  scale_fill_gradient(limits=range(67,11661349),  low="#deebe4", high="#2c3831", na.value="white")
hm_phycar_ind

ggarrange(hm_phycar_com, hm_phycar_ind)
### Pathogen HEatmaps

pathseq_g <- tax_glom(pathcar, "Genus")

taxaorder <- unique(taxa_names(pathseq_g))

pathcar_com <- subset_samples(pathseq_g, breed=="Commercial")
pathcar_ind <- subset_samples(pathseq_g, breed=="Local")

hm_pathcar_com <-plot_heatmap(pathcar_com, taxa.label= "Genus", taxa.order=taxaorder, title = "Commercial", trans = log_trans(10), low="#e3fa9e", high="#59730d", na.value="white")  + theme_classic() +
...
hm_pathcar_ind <-plot_heatmap(pathcar_ind, taxa.label= "Genus", taxa.order=taxaorder,  title = "Local",   trans = log_trans(10), low="#d6f5f4", high="#074e54", na.value="white") +
...
ggarrange(hm_pathcar_com, hm_pathcar_ind)

##### Arrange Figures 

# Arrange and save the multi-panel figure
bac_2 <- ggarrange(bacvenn,bac_stack, bac_nmds, bac_daa, hm_phycar_com, hm_phycar_ind, ncol=3, nrow=2, labels=c("A", "B", "C", "D", "E", " "), font.label = list(size = 70)) 
ggsave(filename = "figures/sup3_bac.png", plot = bac_2, width =150, height = 150, units = "cm", limitsize = FALSE)


# Arrange and save the multi-panel figure
path_2 <- ggarrange(pathvenn,path_stack, path_nmds, path_daa, hm_pathcar_com, hm_pathcar_ind, ncol=3, nrow=2, labels=c("A", "B", "C", "D", "E", " "), font.label = list(size = 70)) 
# Arrange and save the multi-panel figure as a PNG file
ggsave(filename = "figures/sup3_path.png", plot = path_2, width =150, height = 150, units = "cm", limitsize=FALSE)


# Arrange and save the multi-panel figure
arg_2<-ggarrange(argvenn,arg_stack,
                 #arg_nmds, arg_daa, hm_arg_car_com, hm_arg_car_ind, ncol=3, nrow=2, 
                 labels=c("A", "B" # , "C", "D", "E", " "
                          ), font.label = list(size = 70)) 
# Arrange and save the multi-panel figure as a PNG file
ggsave(filename = "figures/sup3_arg.png", plot = arg_2, width =75, height = 75, units = "cm", limitsize=FALSE)

# Arrange and save the multi-panel figure

clin_2 <- ggarrange(# clinvenn, 
  hr_arg_stack, # clin_nmds, clin_daa, hm_clin_mkt_com, hm_clin_mkt_ind,ncol=3, nrow=2,
                    labels=c("A" # , "B"#, "C", "D", "E", " "
                             ), font.label = list(size = 70)) 

ggsave(filename = "figures/sup3_clin.png", plot = clin_2, width =50, height = 50, units = "cm", limitsize=FALSE)
